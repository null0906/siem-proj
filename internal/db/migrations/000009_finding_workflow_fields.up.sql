...
WITH ranked AS (
    SELECT id, ROW_NUMBER() OVER (ORDER BY severity, first_seen) AS row_num
...
SET
    ...
    note = CASE WHEN ranked.row_num % 6 = 0 THEN 'Remediation validation is pending.' ELSE '' END,
    workflow_updated_at = NOW()
FROM ranked WHERE f.id = ranked.id AND ranked.row_num <= 24;

INSERT INTO finding_events (finding_id, event_type, actor, to_value, created_at)
SELECT id, 'assigned', 'system', assignee, workflow_updated_at FROM findings WHERE assignee <> '';
INSERT INTO finding_events (finding_id, event_type, actor, to_value, created_at)
SELECT id, 'due_date_changed', 'system', due_date::text, workflow_updated_at + INTERVAL '1 millisecond' FROM findings WHERE due_date IS NOT NULL;
INSERT INTO finding_events (finding_id, event_type, actor, to_value, created_at)
SELECT id, 'status_changed', 'system', status::text, workflow_updated_at + INTERVAL '2 milliseconds' FROM findings WHERE status = 'in_progress';
INSERT INTO finding_events (finding_id, event_type, actor, note, created_at)
SELECT id, 'note_added', 'system', note, workflow_updated_at + INTERVAL '3 milliseconds' FROM findings WHERE note <> '';
